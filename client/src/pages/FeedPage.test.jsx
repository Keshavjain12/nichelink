...
describe('FeedPage', () => {
  ...
  it('renders posts and requests the selected sort and scope', async () => {
    ...
    const { user } = renderWithProviders(<FeedPage />, {
      preloadedState: authenticatedState({ role: 'FreeMember' }),
    });
    ...
    await waitFor(() =>
      expect(calls.some((call) => call.url.searchParams.get('sort') === 'trending')).toBe(true),
    );

    await user.click(screen.getByRole('radio', { name: 'My communities' }));
    await waitFor(() =>
      expect(calls.some((call) => call.url.searchParams.get('scope') === 'joined')).toBe(true),
    );
  });

  it('optimistically updates the like count for Free members', async () => {
    const { calls } = mockFeed({
      'PUT /api/v1/posts/64b7f0f0f0f0f0f0f0f0f0a1/reactions': apiSuccess({
        liked: true,
        reactionCount: 5,
      }),
    });
    ...
    await waitFor(() =>
      expect(within(article).getByRole('button', { name: /^unlike/i })).toHaveTextContent('5'),
    );
    expect(
      calls.some((call) => call.key === 'PUT /api/v1/posts/64b7f0f0f0f0f0f0f0f0f0a1/reactions'),
    ).toBe(true);
  });
});
