...
const title = z
  .string()
  .trim()
  .min(CONTENT_LIMITS.POST_TITLE_MIN, `Title must be at least ${CONTENT_LIMITS.POST_TITLE_MIN} characters`)
  .max(CONTENT_LIMITS.POST_TITLE_MAX);
const content = z.string().min(1, 'Content is required').max(CONTENT_LIMITS.POST_CONTENT_MAX_HTML, 'Post is too long');
const images = z.array(imageSchema).max(CONTENT_LIMITS.POST_MAX_IMAGES, `At most ${CONTENT_LIMITS.POST_MAX_IMAGES} images`);
...
export const deletePostBody = z.object({ reason: z.string().trim().max(300).optional() }).optional().default({});
