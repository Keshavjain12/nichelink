...
const RICH_TEXT_OPTIONS = Object.freeze({
  allowedTags: ['p', 'br', 'strong', 'em', 'u', 's', 'h2', 'h3', 'blockquote', 'pre', 'code', 'ul', 'ol', 'li', 'a'],
...
const decodeEntities = (text) => text.replace(/&(?:amp|lt|gt|quot|nbsp|#39|#x27);/g, (entity) => ENTITIES[entity]);

export function sanitizeRichText(html) {
  // Quill's semantic HTML encodes every space as &nbsp;, which prevents line wrapping.
  const normalized = html.replace(/&nbsp;|\xa0/g, ' ');
  return sanitizeHtml(normalized, RICH_TEXT_OPTIONS).replace(/<p>\s*<\/p>/g, '').trim();
}
