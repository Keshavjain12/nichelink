...
describe('environment configuration', () => {
  it('accepts a complete production configuration', () => {
    ...
    expect(env).toMatchObject({
      ...
      CLIENT_URL: PRODUCTION.CLIENT_URL,
    });
  ...
  it.each(['', '   '])('treats a blank CLIENT_URL (%j) as missing in production', (value) => {
    expect(() => loadEnv({ ...PRODUCTION, CLIENT_URL: value })).toThrow(
      'CLIENT_URL must be set when NODE_ENV=production',
    );
  });

  it('rejects a non-empty CLIENT_URL that is not a URL', () => {
    expect(() => loadEnv({ ...PRODUCTION, CLIENT_URL: 'not a url' })).toThrow(
  ...
  it('uses the development CLIENT_URL default when it is blank', () => {
    expect(
      loadEnv({ NODE_ENV: 'development', JWT_ACCESS_SECRET: SECRET, CLIENT_URL: '' }).CLIENT_URL,
  ...
  it('parses ALLOW_PRODUCTION_SEED as a boolean defaulting to false', () => {
    ...
    expect(loadEnv({ ...PRODUCTION, ALLOW_PRODUCTION_SEED: 'true' }).ALLOW_PRODUCTION_SEED).toBe(
      true,
    );
  });
