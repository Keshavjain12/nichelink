...
const projectSchema = z
  ...
  .refine((value) => value.remote || value.location.length > 0, {
    path: ['location'],
    message: 'Add a location for on-site projects',
  });
...
function ProjectForm({ project }) {
  ...
  const onSubmit = async (values) => {
    try {
      const saved = project
        ? await updateProject({ id: project.id, ...values }).unwrap()
    ...
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      <Card className="space-y-5 p-5 sm:p-7">
        <TextField
          ...
          placeholder="e.g. Open-source Stripe webhook testing toolkit"
          {...register('title')}
        />
        <TextField
          id="summary"
          ...
          placeholder="Who you're looking for, in a sentence"
          {...register('summary')}
        />
        <TextareaField
          id="description"
          ...
          className="[&_textarea]:min-h-44"
          placeholder="What are you building, where are you now, and what would a collaborator work on?"
        ...
        >
          {(fieldProps) => (
            <Controller
              ...
              name="requiredSkills"
              render={({ field }) => (
                <TagInput {...fieldProps} value={field.value} onChange={field.onChange} max={12} />
              )}
            />
          )}
        </FormField>
...
export default function ProjectEditorPage() {
  ...
  if (!can(PERMISSIONS.PROJECT_CREATE)) {
    return (
      <div className="mx-auto max-w-2xl">
        <UpgradeCallout
          title="Posting projects is a Pro feature"
          description="Upgrade to publish collaboration requests. Anyone can express interest in existing projects."
        />
      </div>
    );
  }
  ...
  if (isEditing && !project.viewer.canEdit)
    return (
      <ErrorState error="Only the author can edit this project." title="Editing isn't available" />
    );
