...
describe('admin API', () => {
  ...
  describe('account management', () => {
    ...
    it('grants and revokes admin access, restoring the subscription-derived role', async () => {
      ...
      await request(app)
        .patch(`/api/v1/admin/users/${pro._id}/admin`)
        .set(bearer(admin))
        .send({ isAdmin: true })
        .expect(200);
  ...
  describe('reports & moderation', () => {
    ...
    const report = (user, body) =>
      request(app).post('/api/v1/reports').set(bearer(user)).send(body);
    ...
    it('validates report targets and prevents duplicates and self-reports', async () => {
      ...
      await report(reporter, {
        targetType: 'Post',
        targetId: '64b7f0f0f0f0f0f0f0f0f0f0',
        reason: 'spam',
      }).expect(404);
      await report(reporter, {
        targetType: 'Post',
      ...
      await request(app)
        .post('/api/v1/reports')
        .send({ targetType: 'Post', targetId: String(post._id), reason: 'spam' })
        .expect(401);
    });

    it('lets admins remove reported content, settling every open report on it', async () => {
      const secondReporter = await createFreeUser();
      const first = await report(reporter, {
        targetType: 'Post',
        targetId: String(post._id),
        reason: 'spam',
        details: 'Affiliate links',
      }).expect(201);
      await report(secondReporter, {
        targetType: 'Post',
        targetId: String(post._id),
        reason: 'off-topic',
      }).expect(201);
      ...
      expect(queue.body.data[0]).toMatchObject({
        ...
        targetOwner: { username: author.username },
      });
      ...
      expect(await Notification.countDocuments({ type: 'moderation', entityType: 'Report' })).toBe(
        2,
      );
    ...
    it('removes reported comments and suspends reported users', async () => {
      ...
      const commentReport = await report(reporter, {
        targetType: 'Comment',
        targetId: String(comment._id),
        reason: 'harassment',
      }).expect(201);
      await request(app)
        .patch(`/api/v1/admin/reports/${commentReport.body.data.id}`)
        .set(bearer(admin))
        .send({ action: 'remove_content' })
        .expect(200);
      expect((await Comment.findById(comment._id).lean()).status).toBe('removed');

      const userReport = await report(reporter, {
        targetType: 'User',
        targetId: String(author._id),
        reason: 'harassment',
      }).expect(201);
      await request(app)
        .patch(`/api/v1/admin/reports/${userReport.body.data.id}`)
        .set(bearer(admin))
    ...
    it('dismisses reports without touching content', async () => {
      const created = await report(reporter, {
        targetType: 'Post',
        targetId: String(post._id),
