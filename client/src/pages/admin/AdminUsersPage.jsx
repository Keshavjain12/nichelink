...
export default function AdminUsersPage() {
  ...
  const { data, isLoading, isFetching, error, refetch } = useListAdminUsersQuery({
    q,
    role,
    status,
    page,
  ...
  const dialogCopy =
    ...
    {
      suspend: {
        title: `Suspend ${action.user.name}?`,
        description: 'They will be signed out everywhere and unable to sign in until reactivated.',
      ...
      grant: {
        title: `Make ${action.user.name} an admin?`,
        description: 'Admins can moderate all content, manage communities and suspend members.',
        confirm: 'Grant admin access',
        variant: 'primary',
      },
      revoke: {
        title: `Remove admin access from ${action.user.name}?`,
        description: 'Their role will return to Free or Pro based on their subscription.',
        confirm: 'Revoke admin access',
        variant: 'danger',
      },
    }[action.type];
        ...
        <Card className="overflow-hidden">
          <div className="relative overflow-x-auto xl:overflow-visible">
            <table className="w-full min-w-[720px] text-left text-sm">
              ...
              <tbody
                className={`divide-y divide-line ${isFetching && !isLoading ? 'opacity-70' : ''}`}
              >
                {isLoading &&
                    ...
                    <tr key={member.id} className="hover:bg-surface-hover/60">
                      ...
                      <td className="px-4 py-3">
                        ...
                        {member.suspensionReason && (
                          <span
                            className="mt-1 block max-w-48 truncate text-xs text-fg-subtle"
                          ...
                          >
                            {member.status === 'active' ? (
                              <MenuItem
                                icon={Ban}
                                danger
                                disabled={member.storedRole === 'Admin'}
                                onClick={() => setAction({ type: 'suspend', user: member })}
                              >
                                Suspend account
                              </MenuItem>
                            ) : (
                              <MenuItem
                                icon={UserCheck}
                                onClick={() => setAction({ type: 'activate', user: member })}
                              >
                                Reactivate account
                              </MenuItem>
                            )}
                            {member.storedRole === 'Admin' ? (
                              <MenuItem
                                icon={ShieldOff}
                                danger
                                onClick={() => setAction({ type: 'revoke', user: member })}
                              >
                                Revoke admin
                              </MenuItem>
                            ) : (
                              member.status === 'active' && (
                                <MenuItem
                                  icon={ShieldCheck}
                                  onClick={() => setAction({ type: 'grant', user: member })}
                                >
                                  Make admin
                                </MenuItem>
                              )
      ...
      >
        {action?.type === 'suspend' && (
          <TextareaField
            ...
            label="Reason (shown to the member when they try to sign in)"
            value={reason}
            maxLength={500}
