...
const projectBody = {
  ...
  description: 'A privacy-first desktop app that turns meeting recordings into searchable summaries.',
...
describe('projects API', () => {
  ...
  it('only allows Pro members to create collaboration requests', async () => {
    ...
    const denied = await request(app).post('/api/v1/projects').set(bearer(free)).send(projectBody).expect(403);
    expect(denied.body.code).toBe('PRO_REQUIRED');

    const res = await request(app).post('/api/v1/projects').set(bearer(pro)).send(projectBody).expect(201);
  ...
  it('filters by skill (case-insensitive), type and free-text search', async () => {
    ...
    await createProject({ author, title: 'Rust database engine', requiredSkills: ['Rust'], projectType: 'startup' });

    const bySkill = await request(app).get('/api/v1/projects?skills=typescript').set(bearer(viewer)).expect(200);
    expect(bySkill.body.data.map((project) => project.title)).toEqual(['Open-source changelog generator']);

    const byType = await request(app).get('/api/v1/projects?projectType=startup').set(bearer(viewer)).expect(200);
    expect(byType.body.data).toHaveLength(1);

    const byText = await request(app).get('/api/v1/projects?q=database').set(bearer(viewer)).expect(200);
  ...
  it('prevents non-authors from editing or deleting, lets admins delete', async () => {
    ...
    await request(app).patch(`/api/v1/projects/${project._id}`).set(bearer(otherPro)).send({ status: 'closed' }).expect(403);
    await request(app).delete(`/api/v1/projects/${project._id}`).set(bearer(otherPro)).expect(403);

    await request(app).patch(`/api/v1/projects/${project._id}`).set(bearer(author)).send({ status: 'closed' }).expect(200);
  ...
  it('handles the interest lifecycle with notifications and privacy', async () => {
    ...
    await request(app).post(`/api/v1/projects/${project._id}/interests`).set(bearer(author)).send({}).expect(400);
    ...
    const interest = await request(app)
      ...
      .expect(201);
    await request(app).post(`/api/v1/projects/${project._id}/interests`).set(bearer(free)).send({}).expect(409);

    expect((await Project.findById(project._id).lean()).interestCount).toBe(1);
    expect(await Notification.countDocuments({ recipient: author._id, type: 'project_interest' })).toBe(1);

    await request(app).get(`/api/v1/projects/${project._id}/interests`).set(bearer(snooper)).expect(403);
    const list = await request(app).get(`/api/v1/projects/${project._id}/interests`).set(bearer(author)).expect(200);
    ...
    const detail = await request(app).get(`/api/v1/projects/${project._id}`).set(bearer(free)).expect(200);
    expect(detail.body.data.viewer).toMatchObject({ interestStatus: 'accepted', canExpressInterest: false });
  });
});
