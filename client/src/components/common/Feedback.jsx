...
export function Spinner({ className, label = 'Loading' }) {
  return (
    <span role="status" className="inline-flex items-center">
      <LoaderCircle className={cn('size-5 animate-spin text-brand-500', className)} aria-hidden="true" />
...
export function InlineAlert({ variant = 'info', title, children, action, className }) {
  ...
  return (
    <div role="status" className={cn('flex flex-wrap items-center gap-3 rounded-xl border px-4 py-3 text-sm', styles[variant], className)}>
