...
const JOBS = [{ name: 'subscription-expiry', intervalMs: HOUR_MS, run: () => expireLapsedSubscriptions() }];

/** Starts in-process interval jobs. Returns a stop function for graceful shutdown. */
export function startJobs() {
  const timers = JOBS.map((job) => {
    const execute = () =>
      job.run().catch((error) => logger.error({ err: error, job: job.name }, 'Scheduled job failed'));
    execute();
