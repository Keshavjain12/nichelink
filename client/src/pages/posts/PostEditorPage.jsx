...
import { EmptyState, ErrorState, InlineAlert, PageLoader, Skeleton } from '../../components/common/Feedback';
...
import { useCreatePostMutation, useGetPostQuery, useUpdatePostMutation } from '../../features/posts/postsApi';
...
const postSchema = z.object({
  ...
  images: z.array(z.object({ url: z.string(), publicId: z.string() }).passthrough()).max(LIMITS.POST_MAX_IMAGES),
});
...
function EditorForm({ existingPost }) {
  ...
  const { data: communities, isLoading: loadingCommunities } = useGetMyCommunitiesQuery(undefined, { skip: isEditing });
  ...
  const values = watch();
  const debouncedDraft = useDebouncedValue(JSON.stringify({ community: values.community, title: values.title, content: values.content, contentText: values.contentText, tags: values.tags }), 800);
  useEffect(() => {
  ...
  if (!isEditing && postable.length === 0) {
    return (
      <EmptyState
        ...
        action={<Button as={Link} to="/communities">Find communities</Button>}
      />
    );
  }
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      <Card className="space-y-6 p-5 sm:p-7">
        {isEditing ? (
          <p className="text-sm text-fg-muted">
            Posting in <span className="font-medium text-fg">{existingPost.community.icon} {existingPost.community.name}</span>
          </p>
        ) : (
          <SelectField id="community" label="Community" required error={errors.community?.message} {...register('community')}>
        ...
        <div>
          <TextField id="title" label="Title" required maxLength={LIMITS.POST_TITLE_MAX} placeholder="What do you want to discuss?" error={errors.title?.message} {...register('title')} />
          <p className="mt-1 text-right text-xs text-fg-subtle">{values.title.length}/{LIMITS.POST_TITLE_MAX}</p>
        </div>
        ...
        <FormField id="tags" label="Tags" hint={`Up to ${LIMITS.POST_MAX_TAGS} topics, e.g. stripe, kubernetes`} error={errors.tags?.message}>
          {(fieldProps) => (
            <Controller
              control={control}
              name="tags"
              render={({ field }) => <TagInput {...fieldProps} value={field.value} onChange={field.onChange} max={LIMITS.POST_MAX_TAGS} />}
            />
          )}
        </FormField>
        ...
        <div>
          ...
          {config?.features.uploads ? (
            <Controller control={control} name="images" render={({ field }) => <ImageUploader value={field.value} onChange={field.onChange} />} />
          ) : (
            <InlineAlert variant="info">Image uploads are not configured on this server.</InlineAlert>
          )}
        </div>
      </Card>
      ...
      <div className="sticky bottom-16 z-10 mt-4 flex justify-end gap-2 rounded-2xl border border-line bg-surface/90 p-3 backdrop-blur lg:bottom-4">
        ...
        <Button type="submit" loading={creating || updating}>{isEditing ? 'Save changes' : 'Publish'}</Button>
      </div>
    </form>
  );
}
...
export default function PostEditorPage() {
  ...
  if (!can(PERMISSIONS.POST_CREATE)) {
    return (
      <div className="mx-auto max-w-2xl">
        <UpgradeCallout title="Publishing is a Pro feature" description="Upgrade to Pro to start discussions with rich text and images." />
      </div>
    );
  }
  ...
  if (isEditing && !post.permissions.canEdit) {
    return <ErrorState error="You can only edit your own published posts." title="Editing isn't available" />;
  }
  ...
  return (
    <div className="mx-auto max-w-3xl space-y-6">
      <PageHeader
        ...
        description={isEditing ? 'Update your post. Readers will see an "edited" label.' : 'Ask a question, share a lesson or start a debate. Drafts are saved on this device.'}
      />
