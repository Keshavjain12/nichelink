  ...
  [
    {
      ...
      senderId: 'u2',
      body: 'Do you take contract work?',
      clientId: null,
      createdAt: '2026-09-16T09:00:00.000Z',
    },
    {
      ...
      conversationId: CONVERSATION_ID,
      senderId: ME,
      body: 'I do — sending samples over.',
      clientId: null,
...
const routes = (overrides = {}) => ({
  ...
  'GET /api/v1/conversations/quota': apiSuccess({
    unlimited: false,
    limit: 10,
    used: 2,
    remaining: 8,
  }),
  [`PATCH /api/v1/conversations/${CONVERSATION_ID}/read`]: apiSuccess({
    conversationId: CONVERSATION_ID,
    unreadCount: 0,
  }),
  ...overrides,
...
describe('ChatPanel', () => {
  it('renders the thread with the participant header', async () => {
    mockApi(routes());
    renderWithProviders(<ChatPanel conversationId={CONVERSATION_ID} />, {
  ...
  it('sends optimistically over REST when the socket is offline', async () => {
    ...
    await waitFor(() =>
      expect(
        calls.some((call) => call.key === `POST /api/v1/conversations/${CONVERSATION_ID}/messages`),
      ).toBe(true),
    );
    // …and is not duplicated once the server confirms it.
    await waitFor(() =>
      expect(screen.getAllByText('Great — what is your availability?')).toHaveLength(1),
    );
  ...
  it('marks a failed send for retry instead of losing the message', async () => {
    mockApi(
      routes({
        [`POST /api/v1/conversations/${CONVERSATION_ID}/messages`]: apiError(
          403,
          'Free members can send 10 messages per day.',
          'DM_LIMIT_REACHED',
        ),
      }),
    );
