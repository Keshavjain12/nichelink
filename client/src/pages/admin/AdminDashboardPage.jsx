  ...
  const content = (
    <Card className="h-full p-4 transition-colors hover:border-line-strong">
      <div className="flex items-center gap-2.5">
        <span
          className={`inline-flex size-8 items-center justify-center rounded-lg ${tones[tone]}`}
        >
        ...
        <ul className="mt-4 space-y-2">
          {segments.map((segment) => (
            <li key={segment.key} className="flex items-center gap-2 text-sm">
              ...
              <span className="w-12 text-right text-xs text-fg-subtle tabular-nums">
  ...
  return (
    <Card>
      ...
      <ul className="divide-y divide-line px-5 pb-2">
        ...
        {data?.items.length === 0 && (
          <li className="py-4 text-sm text-fg-subtle">No admin actions recorded yet.</li>
        )}
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6">
        <StatTile
          ...
          label="Members"
          value={totals.users}
          hint={`+${totals.newUsersThisWeek} this week`}
          to="/admin/users"
        />
        <StatTile
          icon={Crown}
          ...
          hint={`${Math.round((totals.proUsers / (totals.users || 1)) * 100)}% of members`}
          tone="pro"
        />
        <StatTile
          icon={Compass}
          ...
          hint={`${totals.proCommunities} Pro-only`}
        ...
        <StatTile
          ...
          label="Posts"
          value={totals.posts}
          hint={`${formatCompactNumber(totals.comments)} comments`}
        />
        <StatTile
          icon={MessagesSquare}
          label="Messages (7d)"
          value={totals.messagesThisWeek}
          hint={`${formatCompactNumber(totals.conversations)} conversations`}
        />
        <StatTile
          icon={Flag}
          label="Open reports"
          ...
          tone={totals.openReports > 0 ? 'danger' : 'neutral'}
          to="/admin/reports"
        />
      </div>
            ...
            {recentUsers.map((member) => (
              <li key={member.id} className="flex items-center gap-3 py-3">
                ...
                {member.status === 'suspended' ? (
                  ...
                  <Badge variant={member.role === 'ProMember' ? 'pro' : 'neutral'}>
                    {member.role === 'ProMember'
                      ? 'Pro'
                      : member.role === 'Admin'
                        ? 'Admin'
                        : 'Free'}
              ...
              <li key={community.id} className="flex items-center gap-3 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <span className="block text-xs text-fg-subtle">
                    ...
                    {formatCompactNumber(community.postCount)} posts
                  </span>
                </div>
