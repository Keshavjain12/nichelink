...
export async function uploadImage(file, { userId, purpose }) {
  ...
  if (!detectImageMime(file.buffer)) {
    throw new ApiError(400, 'Upload a valid JPEG, PNG, WebP or GIF image', {
...
export function assertOwnedImages(images, userId) {
  ...
  const foreign = images.some(
    (image) => !image.publicId.startsWith(folderPrefix) || !image.url.startsWith(urlPrefix),
  );
