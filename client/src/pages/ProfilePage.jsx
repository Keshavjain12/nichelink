...
function ProfileCommunities({ username }) {
  ...
  if (!isLoading && data.length === 0)
    return <EmptyState icon={Users} title="Not a member of any communities yet" />;
  return (
...
function ProfileProjects({ userId }) {
  const { data, isLoading, error, refetch, hasNextPage, fetchNextPage, isFetchingNextPage } =
    useListProjectsInfiniteQuery({ author: userId, status: 'all' });
  const projects = (data?.pages ?? []).flatMap((page) => page.items);
  if (error) return <ErrorState error={error} onRetry={refetch} />;
  if (!isLoading && projects.length === 0)
    return <EmptyState icon={Briefcase} title="No collaboration requests yet" />;
  return (
    <>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
...
export default function ProfilePage() {
  ...
  if (error)
    return (
      <ErrorState
        error={error}
        title={error.status === 404 ? 'Member not found' : 'Could not load profile'}
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      <Card className="overflow-hidden">
        <div
          className="h-24 bg-linear-to-r from-brand-500/80 via-violet-500/70 to-fuchsia-500/60 sm:h-32"
          aria-hidden="true"
        />
        <div className="px-5 pb-6 sm:px-7">
          <div className="-mt-12 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            ...
            <div className="flex gap-2">
              {profile.isSelf ? (
                <Button as={Link} to="/settings/profile" variant="secondary" leftIcon={Pencil}>
                  Edit profile
                </Button>
              ) : (
                <>
                  <Button as={Link} to={`/messages?to=${profile.id}`} leftIcon={MessagesSquare}>
                    Message
                  </Button>
                  <Menu
                    label="More actions"
          ...
          <div className="mt-4">
            ...
            <div className="mt-3 flex flex-wrap gap-x-5 gap-y-1.5 text-sm text-fg-subtle">
              ...
              <span className="inline-flex items-center gap-1.5">
                ...
                Joined {formatDate(profile.createdAt, { month: 'long', year: 'numeric' })}
              </span>
            </div>
            {profile.bio && (
              <p className="mt-4 max-w-2xl text-sm leading-6 whitespace-pre-line text-fg-muted">
                {profile.bio}
              </p>
            )}
      ...
      <Tabs
        ...
        value={tab}
        onChange={(value) =>
          setSearchParams(value === 'posts' ? {} : { tab: value }, { replace: true })
        }
        tabs={[
